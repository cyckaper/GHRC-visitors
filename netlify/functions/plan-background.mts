import { fail, json, requireAdmin, readJSON, siteUrl } from "../lib/http.mts";
...
export default async (req: Request) => {
  ...
  if (!job) return fail(404, "找不到這個排程工作");
  const visit = normalizeVisit(((job.input || {}) as { visit?: Partial<Visit> }).visit || {}, siteUrl(req));
  ...
  try {
    ...
    await finishJob(jobId, { plan: { ...plan, programme: merged.programme, slides }, visit: merged, estimated_briefing_minutes: Math.round(minutes), master_text_available: !!masterText, warnings });
    return json({ ok: true });
  } catch (e: any) {
    await failJob(jobId, `排程失敗：${e?.message || e}`);
