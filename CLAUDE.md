...
`docs/操作流程.md`（一場參訪從頭到尾：每個步驟與按鈕的目的，＋流程檢討與待補的缺口）。
...
**visits** — `visit_id`、日期、**開始時間、結束時間**（`start_time`／`end_time`；主辦端填的就是這兩個，
`duration_minutes` 由它們算出來給排程、提醒與 ICS 用）、單位、單位類型、國家、人數、主要來賓姓名職稱、
...
- **全站一個「這一場」**：後台最上面一個下拉（`#visitSelect`）＋「已存 14:32」＋「刪掉這一場」，
  五個分頁都在同一場上做事，切分頁時 `reloadTab()` 重載那一頁要的東西。打開後台停在
  **今天或接下來最近**的那一場（那才是現在要做的事），沒有將來的場次就停在乾淨的「＋ 新的一場」。
  **過去的場次不會自己跳出來**——一打開就看到上禮拜那一場的資料，會以為系統把上一次的東西留在畫面上；
...
- **換一場是非同步的**（先把幾個大方框清空，再從伺服器載回來）：載回來的東西一律走 `setField()`——
  **只有「這一格還是我們上次填進去的樣子」才覆寫**，不然會蓋掉人在那零點幾秒空檔裡打的字（測試踩過：
  切換後馬上打逐字稿，字被載入蓋掉）。`selectVisit` 另有一個序號，連續切兩場時前一輪直接作廢，
  免得慢的那一輪後回來、畫面跟上面選的那一場對不起來。
- **表格刪掉一列要自己叫存檔**：刪一列不會冒 input／change 事件（那些 ✕ 走 `[data-del-row]` 的共用處理），
  訪前那兩張表（名單、今日流程）刪完一律 `scheduleSave()`。以前是「畫面上刪掉了、伺服器上還在」——
  重新整理或下一次「AI 查訪客背景」，被刪掉的人又回來。同理，**查背景送的是畫面上現在那一份**
  （`{visit_id, visit}`，`research.mts`／`research-background.mts` 以送上來的那一份為準），
  存檔還沒回來也不會查到舊名單。
...
- **選頁只到區塊，不到單頁**：後台「選用頁次」依 `public/data/slides.json` 的 `groups`（章節／研究室）
  **一個區塊一個勾**，勾了整區一起進去；裡面有哪幾頁點開來看，但那只是說明，不是可勾的東西。
  要決定的是「這一場要不要講 302 規劃室」，不是二十幾個核取方塊；母簡報改版時比對的也是區塊、不是頁碼。
  `required: true` 的區塊（必選頁：封面、流程、架構、核心宣稱／謝謝）鎖住、永遠在。
  每一頁必須恰好屬於一個區塊。存回 `visit.slides` 的仍然是**頁次**（產檔、`slide_performance`、history 都照舊），
  由 `lib/visit.mjs snapSlidesToGroups()` 把選擇展開成整區——**AI 排程挑的頁也走同一個函式**
  （`plan-background`），所以「也挑了 N 頁」跟簡報分頁上看到的是同一件事；提示詞也明說「以區塊為單位思考」。
