...
> **Claude 讀一封長信＋附件也常常超過一般函式的 10 秒**（實際踩過：畫面回 504 Inactivity Timeout）。
> 所以 `/api/extract` 只做檢查與轉檔，接著開一個工作、觸發背景函式 `extract-background`（15 分鐘上限），
> 前端每 3 秒問一次 `GET /api/extract?job=<id>`（`running`／`done`／`error`，`done` 才填表）。
> 工作本身在 `netlify/lib/jobs.mts`：`jobs/<id>.json` 存狀態**與輸入**（信件全文、轉好的附件），
> 背景函式自己去讀，觸發只帶 `job_id`——所以抽取不必先存檔，而且輪詢不會把附件再送回前端一次。
> **排行程（`/api/plan`）同一套**（提示詞帶整份頁次索引與母簡報文字，更慢）：`plan-background` ＋ `GET /api/plan?job=<id>`。
> 後台的共用輪詢是 `admin.html` 的 `waitForJob(path, jobId, infoId)`。再有跑得久的 AI 就照這個模式加，
> 不要直接在一般函式裡等。
