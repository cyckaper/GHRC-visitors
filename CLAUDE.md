...
> **名單上每一個人有兩種身分，分開記**：`role: "lead"` 是**主賓**（禮賓身分，封面與信件開頭寫的那個人），
> `contact: true` 是**主要聯絡人**（負責收信、確認行程的那個人）。協調參訪的人不一定等於參加參訪的人，
> 所以**確認信預設只寄給 contact**、感謝信才寄給名單上每一位。抽取時 AI 就標出 contact（寫信的人、信裡指定的承辦人）。

> **已經交出去的東西會不會過期**：網頁會跟著更新，**已經寄出的確認信與下載的 .pptx 不會**。
> 產檔與寄出確認信時把當下的**行程指紋**記進 `deck.fingerprint`／`letters.confirmation.fingerprint`
> （`lib/visit.mjs scheduleFingerprint`／`deckFingerprint`，指紋由**伺服器**蓋，算法只有一份），
> 之後對不上就由 `staleOutputs()` 算出來、跟著 `GET`／`POST /api/visits` 回一個 `stale`：
> 進度線把綠勾收回來改說「簡報 需重產」「確認信 行程已改」，簡報分頁與確認信卡片各說一句為什麼。

> **後續那四件事可以標「本次沒有」**（`visit.wrapup.na`）：沒有簽名簿、沒交換名片、沒錄口述，
> 參訪一樣走完了，感謝信一樣該寄。標了就不算未完成——進度線顯示「本次沒有」（`–`，不是灰點），
> `reminder-cron` 也不再為它寄信（`wrapupSettled()`）。做到了以做到的為準。
...
- **兩封信各在它該在的時間點**（明確指示，沒有獨立的「信件」分頁）：**確認信在「訪前」**（排完行程、
  ...
  看不到自己勾了什麼。**預設勾的人也不一樣**：確認信只勾**主要聯絡人**（`guest.contact`；名單上一個都沒標
  才退回全選——寧可多寄，也不要按了寄出卻一個人都沒寄到），感謝信勾名單上每一位。
  寄出走同一支 `letter` 的 `action:"send"`，只差 `kind`。畫面會依日期標出「現在該寄這一封」。
  **兩顆寄出鍵上直接寫「寄出○○信（N 位）」、旁邊列出名字**——「寄出」兩個字看不出寄給誰，
  勾錯對象要在按下去之前就看得出來。
...
- **回饋的來源分開算**（明確要求）：「主持人覺得對方有合作興趣」**不等於**「對方說他想合作」。
  ...
  合併成一個數字之後就再也分不出來了，而這一格正是「回饋下一次簡報」要拿來當依據的。
...
- **行程只有一張表**（明確要求）：以前是「今日流程」排時間區塊、底下另一張「現場動線」再填每一間幾分鐘——
  同一件事要看懂兩套、改兩個地方。現在每一間的分鐘數就長在「研究室參訪」那一列底下（`roomsRow()`），
  總體介紹的地點在 briefing 那一列，**分鐘一改就把後面各段的時間往後推算一次**（`retimeProgramme()`）：
  時間是算出來的，不是第二個要填的欄位。表底下一行說「流程排了 N 分」，跟「幾點到幾點」差多少直接講。
