...
/**
 * 依訪前排程算出動線各步的預定進入時間。
 * 動線第一步固定是總體介紹（room="briefing"）：起點 = briefing 區塊的 start，否則 = visit.start_time；
 * 若動線裡沒有 briefing（舊資料），起點 = tour 區塊的 start。
 */
export function plannedEntries(visit) {
  const date = visit.date;
  const programme = visit.programme || [];
  const steps = visit.itinerary || [];
  const startsWithBriefing = steps.length > 0 && String(steps[0].room) === "briefing";
  const briefingBlock = programme.find((p) => p.kind === "briefing");
  const tour = programme.find((p) => p.kind === "tour");
  const startHHMM = startsWithBriefing ? briefingBlock?.start || visit.start_time || "10:00" : tour?.start || visit.start_time || "10:00";
