...
| 階段 | 誰 | 做什麼 | 在哪 |
|---|---|---|---|
| 訪前 | 承辦 | 貼上 email 往來、或上傳名單檔（Word／Excel／CSV／PDF／照片）→ AI 抽出單位、名單（含隨行者 email）、目的、興趣 → 確認 → **AI 查訪客背景**（網路上的公開資料 → 可能的參訪目的、可能最想看哪幾間、可以先準備什麼）→ AI 排行程（總體介紹在 302）→ 儲存 → 拿到專屬網址與 QR、確認信草稿、後續提醒 .ics | `admin.html`「訪前」 |
...
| 後續 | 主持人 | 拍一張簽名簿（AI 讀字）＋ 三十秒口述（轉文字、抽取），一分鐘；再把簡報 PDF、合照、相關連結放上專屬頁面 | `admin.html`「後續」（.ics 提醒直達） |
...
netlify/functions/ API（.mts）：visits extract plan letter respond signbook cards transcribe summary media materials translate master session drive
                   跑得久的 AI 一律走背景函式（一般函式只有 10 秒）：extract- plan- research- letter- summary- signbook- transcribe- cards- translate-background
                   另有 drive-sync-background drive-cron
netlify/lib/       函式共用：store（file／blobs／sheets）、ai（Claude、Whisper、mock）、http、data、types、jobs（背景工作）
lib/               純 JS 共用：visit（id、匿名化、寄送清單、ICS、開始／結束時間）
cli/               npm run deck：同一份核心的本機入口 → pptx（＋PDF）
scripts/           slim-master.py（母簡報瘦身）、make-world.mjs（訪客地圖）、i18n-scan.mjs（後台英文對照）、make-fixture.py（測試用合成簡報）、dev-server.mjs
...
```bash
npm install
cp .env.example .env            # 填 ANTHROPIC_API_KEY、ADMIN_TOKEN；沒有金鑰就設 AI_MOCK=1
set -a; . ./.env; set +a
